...
def reward_helper_agg(H,p,N,noise_var,Pmax,neighbors_in):
    # Take pow 2 of abs_H, no need to take it again and again
    H_2 = H ** 2
    #H = H.reshape(N,N)
    # Diagonals are self sum_rate
    ...
    for loop in range (N):
        ...
        sum_rate[loop,neighbors_in[-1][loop]] = -((np.log2(1+np.minimum(treshold_sinr,tmp_1/np.maximum(noise_var,([tmp_2]*(len(neighbors_in[-1][loop]))-neighbors_in[-2][loop]* p[neighbors_in[-1][loop]])))))-(np.log2(1+np.minimum(treshold_sinr,tmp_1/tmp_2))))
...
def actor_net2(x, weights, biases): # Create model
    layer_1 = tf.add(tf.matmul(x, weights['h1']), biases['b1'])   # x = w*x+b
    layer_1 = tf.nn.relu(layer_1)                                 # x = max(0, x)
    layer_2 = tf.add(tf.matmul(layer_1, weights['h2']), biases['b2'])
    layer_2 = tf.nn.relu(layer_2)
    ...
    out_layer = tf.nn.sigmoid(out_layer)
    # out_layer = tf.nn.relu6(out_layer) / 6.
    return tf.math.divide(out_layer , tf.maximum(tf.ones(1), tf.reduce_sum(out_layer)))
